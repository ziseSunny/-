...
D5 = np.zeros((1,365))
#最终结果
calculate(S1,E1,I1,R1,D1,w1)
calculate(S2,E2,I2,R2,D2,w2)
calculate(S3,E3,I3,R3,D3,w3)
calculate(S4,E4,I4,R4,D4,w4)
calculate(S5,E5,I5,R5,D5,w5)
#求对应的指标
max1 = np.max(I1[0,:])
max2 = np.max(I2[0,:])
max3 = np.max(I3[0,:])
...
with open('医疗援助前的隔离效果的敏感性分析.txt','w') as f:
    f.write('援助前的隔离效果为%.2f'%w1+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max1),c1))
    f.write('援助前的隔离效果为%.2f'%w2+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max2),c2))
    f.write('援助前的隔离效果为%.2f'%w3+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max3),c3))
    f.write('援助前的隔离效果为%.2f'%w4+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max4),c4))
    f.write('援助前的隔离效果为%.2f'%w5+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max5),c5))
f.close()
...
plt.legend(['w0=1.5','w0=1.75','w0=2','w0=2.25','w0=2.5'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('医疗援助前的隔离效果对于患者数量的影响.png')
...
plt.plot(time[0,:],I3[0,:],color='red')
plt.plot(time[0,:],I4[0,:],color='skyblue')
plt.legend(['w0=1.5','w0=1.75','w0=2','w0=2.25'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('医疗援助前的隔离效果对于患者数量的影响_clear.png')