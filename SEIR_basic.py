...
for i in range(46-1,46+12-1):
    S[0][i+1] = S[0][i] - (prob * n2)*((E[0][i]*S[0][i])/people) - (prob * n2)*((I[0][i]*S[0][i])/people)
    E[0][i+1] = E[0][i] + (prob * n2)*((E[0][i]*S[0][i])/people) + (prob * n2)*((I[0][i]*S[0][i])/people) - alpha*E[0][i]
    I[0][i+1] = I[0][i] + alpha*E[0][i] - gamma*I[0][i] - d*I[0][i]
    R[0][i+1] = R[0][i] + gamma*I[0][i]
    D[0][i+1] = D[0][i] + d*I[0][i]
...
plt.ylabel('人数/人')
plt.savefig('引入医疗后的SEIR模型对应的患者人数趋势图.png')
#求解疫情基本得到控制的时间和患病总人数
control_limit = 50
max_I = np.max(I[0,:])
index_max = 0
