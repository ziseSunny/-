...
d_4 = 51
d_5 = 56
def calculate(S,E,I,R,D,d2):
    S[0][0] = people - 1
    E[0][0] = 0
    I[0][0] = 1
    R[0][0] = 0
    D[0][0] = 0
    d1 = 46 #1.22及之前均未进行自我防护(45:12.8~1.22)
...
with open('自我防护时间的敏感性分析.txt','w') as f:
    f.write('%d天自我防护，最大值为%d，基本控制需要的时间为第%d天\n'%(d_1,round(max1),c1))
    f.write('%d天自我防护，最大值为%d，基本控制需要的时间为第%d天\n'%(d_2,round(max2),c2))
    f.write('%d天自我防护，最大值为%d，基本控制需要的时间为第%d天\n'%(d_3,round(max3),c3))
    f.write('%d天自我防护，最大值为%d，基本控制需要的时间为第%d天\n'%(d_4,round(max4),c4))
    f.write('%d天自我防护，最大值为%d，基本控制需要的时间为第%d天\n'%(d_5,round(max5),c5))
...
plt.legend(['d2=36','d2=41','d2=46','d2=51','d2=56'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('自我防护时间对于患者数量的影响.png')

plt.figure(2,figsize=(8,6))
plt.plot(time[0,:],I1[0,:],color='orange')
plt.plot(time[0,:],I2[0,:],color='hotpink')
plt.plot(time[0,:],I3[0,:],color='red')
plt.legend(['d2=36','d2=41','d2=46'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('自我防护时间对于患者数量的影响_clear.png')