...
D5 = np.zeros((1,365))
#最终结果
calculate(S1,E1,I1,R1,D1,l_1)
calculate(S2,E2,I2,R2,D2,l_2)
calculate(S3,E3,I3,R3,D3,l_3)
calculate(S4,E4,I4,R4,D4,l_4)
calculate(S5,E5,I5,R5,D5,l_5)
#求对应的指标
max1 = np.max(I1[0,:])
max2 = np.max(I2[0,:])
max3 = np.max(I3[0,:])
...
with open('自我防护的效果的敏感性分析.txt','w') as f:
    f.write('自我防护效果为%.2f'%l_1+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max1),c1))
    f.write('自我防护效果为%.2f'%l_2+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max2),c2))
    f.write('自我防护效果为%.2f'%l_3+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max3),c3))
    f.write('自我防护效果为%.2f'%l_4+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max4),c4))
    f.write('自我防护效果为%.2f'%l_5+'最大值为%d，基本控制需要的时间为第%d天\n'%(round(max5),c5))
f.close()
...
plt.legend(['l=0.6','l=0.65','l=0.7','l=0.75','l=0.8'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('自我防护的效果对于患者数量的影响.png')