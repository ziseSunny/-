...
with open('小区管控时间的敏感性分析.txt','w') as f:
    f.write('%d天小区管控，最大值为%d，基本控制需要的时间为第%d天\n'%(d_1,round(max1),c1))
    f.write('%d天小区管控，最大值为%d，基本控制需要的时间为第%d天\n'%(d_2,round(max2),c2))
    f.write('%d天小区管控，最大值为%d，基本控制需要的时间为第%d天\n'%(d_3,round(max3),c3))
    f.write('%d天小区管控，最大值为%d，基本控制需要的时间为第%d天\n'%(d_4,round(max4),c4))
    f.write('%d天小区管控，最大值为%d，基本控制需要的时间为第%d天\n'%(d_5,round(max5),c5))
...
plt.legend(['d3=55','d3=60','d3=65','d3=70','d3=75'])
plt.xlabel('距离2019.12.8的时间')
plt.ylabel('人数/人')
plt.savefig('小区管控时间对于患者数量的影响.png')